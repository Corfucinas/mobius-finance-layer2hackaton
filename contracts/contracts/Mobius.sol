...
contract Mobius is Pausable, Importable, IMobius {
    ...
    function mintFromCoin(uint256 collateralRate) external override payable returns (bool) {
        _stake(nativeCoin, USD, msg.value);
        _mint(nativeCoin, msg.value, USD, collateralRate);
        return true;
    }

    function mintFromToken(bytes32 stake, uint256 amount, uint256 collateralRate) external override returns (bool) {
        require(stake != nativeCoin, 'Mobius: Native Coin use "mintFromCoin" function');

        _stake(stake, USD, amount);
        _mint(stake, amount, USD, collateralRate);
        return true;
    }

    function mintFromPreviousStake(bytes32 stake, uint256 amount) external override returns (bool) {
        Issuer().issueDebtWithPreviousStake(stake, msg.sender, USD, amount);
        emit Minted(msg.sender, stake, 0, amount);
    }

    function shortFromCoin(bytes32 debtType, uint256 collateralRate) external override payable returns (bool) {
        _stake(nativeCoin, debtType, msg.value);
        _mint(nativeCoin, msg.value, debtType, collateralRate);
        return true;
    }

    function shortFromToken(bytes32 stake, uint256 amount, bytes32 debtType, uint256 collateralRate) external override returns (bool) {
        require(stake != nativeCoin, 'Mobius: Native Coin use "shortFromToken" function');

        _stake(stake, debtType, amount);
        _mint(stake, amount, debtType, collateralRate);
        return true;
    }

    function shortFromPreviousStake(bytes32 stake, bytes32 debtType, uint256 amount) external override returns (bool) {
    ...
    function _mint(
        ...
        uint256 issueDebtInSynth = issueAmountInUSD;

        // trading fee charge
        if (debtType != USD) {
            issueDebtInSynth = issueAmountInUSD.decimalDivide(AssetPrice().getPrice(debtType));
        }

        Issuer().issueDebt(stake, msg.sender, debtType, issueAmountInUSD, issueDebtInSynth);
    ...
    function burn(bytes32 stake, bytes32 debtType, uint256 amount, bool withdraw) external override notPaused returns (bool) {
        ...
        if (withdraw) {
            uint256 claimable = Staker().getClaimable(stake, msg.sender, debtType, beforeCollateralRate);
    ...
    function stakeFromCoin(bytes32 debtType) external override payable returns (bool) {
    ...
    function stakeFromToken(bytes32 stake, bytes32 debtType, uint256 amount) external override returns (bool) {
        require(stake != nativeCoin, 'Mobius: Native Coin use "mintFromCoin" function');
