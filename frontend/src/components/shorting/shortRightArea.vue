<template>
    <div class="components-mint-right">
        <div class="header" v-if="hidedenTitle">{{$t('mintAndBurn.min')}} ETH {{$t('mintAndBurn.by')}} {{$t('mintAndBurn.staking')}} moUSD</div>
        ...
        <div class="content">
            <div class="header">
                <span>{{formModel.syntheticAssetsCoinName}} {{$t('mintAndBurn.price')}}</span>
                <span><a-statistic :value="filterFormerAccuracy(formModel.syntheticAssetsOrcalPrice)" :precision="4" style="margin-right: 5px" />moUSD</span>
            </div>
            ...
            <div class="area">
                <div class="item">
                    ...
                    <div class="amount">
                        ...
                        <a-statistic :value="formModel.stakeAmount" :precision="4" style="margin-right: 5px" /> {{formModel.coinNmae}}
                    </div>
                </div>

                <div class="item">
                    <div class="label">{{$t('shorting.new')}} {{formModel.syntheticAssetsCoinName}} {{$t('shorting.debt')}}</div>
                    <div class="amount">
                        <a-statistic v-if="filterFormerAccuracy(formModel.oldAvailableAmount)" :value="formModel.oldAvailableAmount" :precision="4" /> 
                        <img v-if="formModel.oldAvailableAmount" style="height: 15px;margin: 0 5px;" src="../../assets/img/right.png" />
                        <a-statistic :value="filterFormerAccuracy(formModel.availableAmount)" :precision="4" style="margin-right: 5px" /> {{formModel.syntheticAssetsCoinName}}
                    </div>
                </div>
                ...
                <div class="item">
                    ...
                    <div class="amount"><a-statistic :value="formModel.liquidationLinRate" :precision="2" suffix="%" style="margin-right: 5px" /> </div>
                </div>
...
export default {
    ...
    data () {
        return {
            formModel: {
                ...
                liquidationLinRate: '',
                rateStatus: 1,
                syntheticAssetsCoinName: '',
                oldStakeAmount: 0,
                oldAvailableAmount: 0,
    ...
    methods: {
        updateInfo ({ coinNmae, liquidationPrice, liquidationRate, rateStatus, lin, mousdBalance, availableAmount, 
            stakeAmount, safeRate, syntheticAssetsCoinName, oldAvailableAmount, oldStakeAmount, syntheticAssetsOrcalPrice }) {
            ...
            this.formModel.syntheticAssetsCoinName = syntheticAssetsCoinName
            this.formModel.oldAvailableAmount = oldAvailableAmount
            this.formModel.oldStakeAmount = oldStakeAmount
            this.formModel.syntheticAssetsOrcalPrice = syntheticAssetsOrcalPrice
        },
