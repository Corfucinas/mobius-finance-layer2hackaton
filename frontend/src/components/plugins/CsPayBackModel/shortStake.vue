        ...
        <div class="modal-content">
            <div class="debt-content-item">
                <div class="content-item">
                    ...
                    <div class="inp-blocl">
                        <div class="inp-coin">
                            <a-input-number class="inp" v-model="formModel.stakeAmount" :min="0" placeholder="0.0" :precision=4 />
                            <div class="coin-but" @click="maxClick('stake')">{{$t('modal.max')}}</div>
                        </div>
                        <a-button :class="formModel.stakeAmount ? 'butShort' : 'disableShortBut'"
                            :disabled="!formModel.stakeAmount" @click="showModalClick('Staked')">
                ...
                <div class="content-item">
                    ...
                    <div class="inp-blocl">
                        <div class="inp-coin">
                            <a-input-number class="inp" v-model="formModel.unStakeAmount" :min="0" placeholder="0.0" :precision=4 />
                            <div class="coin-but" @click="maxClick('unStake')">{{$t('modal.max')}}</div>
                        </div>
                        <a-button :class="formModel.unStakeAmount ? 'butShort' : 'disableShortBut'"
                            :disabled="!formModel.unStakeAmount" @click="showModalClick('Unstaked')">
...
<script>
import { getStaked, getCollateralRate, getClaimable,
    getDebt, getIssuable, getLiquidationRate, getPrice, getBalance, stakeFromCoin, approve, stakeFromToken, claim } from '@/config/ContractMethods'
...
export default {
    ...
    methods: {
        show (row) {
            ...
            this.config.title = ` ${this.formModel.currentCoinName} ` + this.$t('debtManagement.staked')
            this.initData()
        ...
        async getData () {
            ...
            this.formModel.unStakeBalance = await getClaimable(this.formModel.currentCoinName, this.account, this.formModel.debtCoinName, collateralRate)
        ...
        showModalClick (type) {
            ...
            if (type === 'Staked') {
                ...
                collateralizationRatio  = toFixedVal(amountAndBalance * this.formModel.currentPrice / (this.formModel.debtPrice * this.formModel.moTslaPrice))
            ...
            try {
                ...
                if (this.formModel.clickType === 'Staked') {
                    if (this.formModel.currentCoinName === 'MATIC') {
                        resultHash = await stakeFromCoin(this.formModel.debtCoinName, this.formModel.stakeAmount)
                        ...
                        if (hash) {
                            this.provider.once(hash, async (transaction) => {
                                console.log('transaction===', transaction)
                                resultHash = await stakeFromToken(this.formModel.currentCoinName, this.formModel.debtCoinName, this.formModel.stakeAmount)
                                if (resultHash) {
                                    this.listenerAccountbalance(resultHash)
                                }
                            })
                    ...
                    resultHash = await claim(this.formModel.currentCoinName, this.formModel.debtCoinName, this.formModel.unStakeAmount)
                    if (resultHash) {
                        this.listenerAccountbalance(resultHash)
                    }
                }
